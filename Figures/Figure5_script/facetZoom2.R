# Idea Credit:
# https://stackoverflow.com/questions/52665619/how-to-change-the-position-of-the-zoomed-area-from-facet-zoom/52686856#52686856
# Source code from github: https://github.com/thomasp85/ggforce/blob/main/R/facet_zoom.R
...
#' @export
FacetZoom2 <- ggproto('FacetZoom2', Facet,
                     compute_layout = function(data, params) {
                       layout <- ggforce:::data_frame0(
                     ...
                     draw_panels = function(self, panels, layout, x_scales, y_scales, ranges, coord,
                       ...
                       if ('full' %in% layout$name && params$split) {
                         ...
                         final <- gtable::gtable_add_grob(final, list(indicator_h, indicator_h), c(2, 6), 3,
                                                  c(2, 6), 5, z = -Inf, name = 'zoom-indicator')
                         final <- gtable::gtable_add_grob(final, list(indicator_v, indicator_v), 3, c(2, 6),
                         ...
                         if (params$horizontal) {
                           ...
                           #final <- gtable::gtable_add_cols(panelGrobs[[2]], space)
                           #final <- cbind(final, panelGrobs[[1]], size = 'first')
                           final$heights <- heights
                           ...
                           final <- gtable::gtable_add_grob(final, indicator_h, 2, 3, 2, 5, z = -Inf,
                                                    name = 'zoom-indicator')
                         } else {
                           ...
                           final <- gtable::gtable_add_grob(final, indicator_v, 3, 2, 5, z = -Inf,
                                                    name = 'zoom-indicator')
                         }
                       }
                       final
...
create_panels <- function(panels, x.axis, y.axis) {
  Map(function(panel, x, y, i) {
    ...
    table <- gtable::gtable(widths, heights)
    table <- gtable::gtable_add_grob(table, panel, t = 2, l = 2, z = 2, clip = 'on',
                             name = paste0('panel-', i))
    table <- gtable::gtable_add_grob(table, x, t = c(1, 3), l = 2, z = 4, clip = 'off',
                             name = paste0(c('axis-t-', 'axis-b-'), i))
    table <- gtable::gtable_add_grob(table, y, t = 2, l = c(1, 3), z = 4, clip = 'off',
